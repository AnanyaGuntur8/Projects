public class Communities {
	...
	int pm_25columnIndex = HLT_PM2_5;
    if (commnunity >= 0 && commnunity< data.length){
        return data[commnunity][pm_25columnIndex];

    }
    return 0.0; //find out what this does. (double)Integer.MIN_VALUE)
    }
    ...
    int LifeExpectancy = HLT_LIFE_EXPECT;
    double totalvalue = 0.0;
    for(double[] communityData : data){
        totalvalue += communityData[LifeExpectancy];
    }
    double avglifeExpectancy = totalvalue/data.length;
    return avglifeExpectancy; //want to return the value of life expectancy 
    }
    ...
    int healthissueIndex = 0;
    if (healthIssue == 'A'){
        healthissueIndex = HLT_ASTHMA;
    } else if (healthIssue == 'D'){
        healthissueIndex = HLT_DIABETES;
    } else {
        healthissueIndex = HLT_HEART_DISEASE;

    }

    double highestPopulation = 0.0;
    for(double[] communitydata: data){
        double index = communitydata[healthissueIndex];
        if (index> highestPopulation){
            highestPopulation = index;
    ...
    public static int numberOfCommunities (double[][] data, int columnIndex, double percentageThreshold, boolean isDisadvantage) {
        ...
        if (isDisadvantage == true) {
            ...
            for (int i =0; i<data.length; i++){
                if (data[i][21] ==0 &&data[i][columnIndex]*100>=percentageThreshold){
                    count += 1;
            }
        }
        }
    ...
    public static int mostPopulatedDisadvantagedWithLowIncome(double[][] data) {
        ...
        return populated; // replace this line
    }
